...
def heston_model_sim(S0, v0, rho, kappa, theta, sigma, T, N, M):
    """
    Inputs:
    - S0, v0: inital parameters for asset and variance
    - rho   : correlation between asset returns and variance
    - kappa : rate of mean reversion of variance
    - theta : long-term mean of variance
    - sigma : vol of vol / volatility of volatility process
...
fig, ax = plt.subplots()
ax = sns.kdeplot(S_p[-1], label=r"ρ=0.98", ax=ax)
ax = sns.kdeplot(S_n[-1], label=r"ρ=−0.98", ax=ax)
